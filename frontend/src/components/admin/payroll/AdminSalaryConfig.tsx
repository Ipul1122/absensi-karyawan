...
export default function AdminSalaryConfig({ token }: AdminSalaryConfigProps) {
  ...
  const handleOpenWaModal = (employee: User) => {
    ...
    if (cfg) {
      ...
      const meal = (isPending && cfg.pending_allowance_meal_daily !== null && cfg.pending_allowance_meal_daily !== undefined ? cfg.pending_allowance_meal_daily : cfg.allowance_meal_daily) ?? 0
      const trans = (isPending && cfg.pending_allowance_transport_daily !== null && cfg.pending_allowance_transport_daily !== undefined ? cfg.pending_allowance_transport_daily : cfg.allowance_transport_daily) ?? 0
      const pos = (isPending && cfg.pending_allowance_position !== null && cfg.pending_allowance_position !== undefined ? cfg.pending_allowance_position : cfg.allowance_position) ?? 0
      ...
      const absence = (isPending && cfg.pending_deduction_absence_daily !== null && cfg.pending_deduction_absence_daily !== undefined ? cfg.pending_deduction_absence_daily : cfg.deduction_absence_daily) ?? 0
