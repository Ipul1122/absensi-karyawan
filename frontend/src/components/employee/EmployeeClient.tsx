...
export default function EmployeeClient({
  ...
  if (todayAttendance && todayAttendance.clock_in && todayAttendance.attendance_type !== 'client') {
    return (
      <div className="space-y-6">
        <section className="bg-white border border-orange-100 rounded-3xl p-8 text-center shadow-sm flex flex-col items-center justify-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-800 font-quicksand">Absen Kunjungan Klien Tidak Aktif</h3>
            <p className="text-sm text-slate-500 mt-2 max-w-md mx-auto leading-relaxed font-medium font-quicksand">
              Hari ini Anda tercatat melakukan absensi tipe <strong className="capitalize">{todayAttendance.attendance_type === 'kantor' ? 'Absen Kantor' : 'Kunjungan Kerja / Lapangan'}</strong>.
              <br />
              Silakan buka halaman absensi yang sesuai untuk melihat detail atau melakukan absen keluar.
            </p>
          </div>
          <NavLink
            to={todayAttendance.attendance_type === 'kantor' ? '/employee/absen' : '/employee/sales'}
          ...
          >
            Buka Absen {todayAttendance.attendance_type === 'kantor' ? 'Kantor' : 'Kunjungan Lapangan'}
          </NavLink>
        </section>
      </div>
    )
  ...
  const handleAttendanceSubmit = async (type: 'check-in' | 'check-out') => {
    ...
    if (type === 'check-in' && !notes) {
      Swal.fire({
        title: 'Catatan Klien Wajib',
        text: 'Silakan sebutkan nama klien / mitra kerja yang Anda temui pada kolom catatan.',
        icon: 'warning',
        background: '#1e293b',
        color: '#f8fafc',
        confirmButtonColor: '#6366f1'
    ...
    <div className="space-y-6">
      ...
      <div className="flex bg-orange-50/30 border border-orange-100 rounded-2xl p-1.5 backdrop-blur-xl">
        ...
        >
          ...
          Absen Masuk Klien
          {todayAttendance?.clock_in && todayAttendance.attendance_type === 'client' && (
        ...
        >
          <Clock className="w-4.5 h-4.5" />
          Absen Keluar Klien
          {todayAttendance?.clock_out && todayAttendance.attendance_type === 'client' && (
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          )}
        </button>
      </div>
          ...
          <section className="bg-white border border-orange-100 rounded-3xl p-6 shadow-sm space-y-6">
            <div className="border-b border-orange-100 pb-3 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2 font-quicksand">
                  ...
                  Presensi Masuk: Kunjungan Klien (Client Visit)
                </h2>
                ...
                <div className="space-y-2">
                  ...
                  {latitude && longitude && (
                    <div className="p-3 rounded-xl border text-[11px] font-bold flex items-start gap-2 leading-relaxed text-emerald-700 bg-emerald-50 border-emerald-200">
                      ...
                      <span>Lokasi siap untuk Kunjungan Klien (Jarak ke kantor: {currentDistance !== null ? `${Math.round(currentDistance)}m` : 'mengalkulasi...'}). Radius kantor dinonaktifkan.</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-2 pt-2 border-t border-orange-100">
              <label className="block text-xs font-bold text-slate-550 uppercase tracking-wider flex items-center gap-1 font-quicksand">
                ...
                Catatan Kunjungan Klien (Nama Klien / Mitra Kerja)*
              </label>
              <textarea placeholder="Wajib menuliskan nama klien, instansi, atau mitra kerja yang dikunjungi beserta tujuannya..." value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} className="w-full bg-slate-50 border border-slate-200 focus:border-red-500 text-slate-800 placeholder-slate-450 rounded-xl py-2.5 px-4 outline-none transition-all text-xs resize-none font-medium font-quicksand" required />
            </div>

            <div className="pt-2 flex justify-end">
              <button onClick={() => handleAttendanceSubmit('check-in')} disabled={submitting || !capturedPhoto || !latitude || !longitude || !notes} className="px-6 py-3 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-2xl transition-all shadow-lg shadow-orange-600/10 cursor-pointer text-sm flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed font-quicksand">
                {submitting ? (
                  <>
                  ...
                  <>
                    <Check className="w-4 h-4" /> Kirim Absen Masuk Klien
                  </>
                )}
              </button>
            </div>
          ...
          <section className="bg-white border border-orange-100 rounded-3xl p-6 shadow-sm space-y-6 relative overflow-hidden">
            ...
            <div className="flex justify-between items-center pb-3 border-b border-orange-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800 font-quicksand">Absen Klien Masuk Tercatat!</h2>
                  <p className="text-xs text-slate-500">Anda sudah absen masuk kunjungan klien hari ini.</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 pt-2">
              <div className="md:col-span-7 space-y-4">
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
                  <div>
                    <span className="text-[10px] font-bold text-slate-550 uppercase tracking-wider block font-quicksand">Waktu Masuk Klien</span>
                  ...
                  <div className="grid grid-cols-3 gap-4 pt-2 border-t border-slate-200">
                    <div>
                      <span className="text-[10px] font-bold text-slate-550 uppercase tracking-wider block font-quicksand">Tipe Presensi</span>
                  ...
                  {todayAttendance.notes_in && (
                    <div className="pt-2 border-t border-slate-200">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block font-quicksand">Klien / Detail Kunjungan</span>
          ...
          <section className="bg-white border border-orange-100 rounded-3xl p-12 text-center shadow-sm flex flex-col items-center justify-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-2 max-w-md mx-auto leading-relaxed font-medium">
                Anda harus melakukan **Absen Masuk (Check-In)** Kunjungan Klien terlebih dahulu sebelum dapat mencatat Absen Keluar (Check-Out).
              </p>
            </div>
          </section>
        ) : !todayAttendance.clock_out ? (
          todayAttendance.attendance_type === 'client' && !isVisitEnded ? (
            <section className="bg-white border border-orange-100 rounded-3xl p-8 text-center shadow-sm flex flex-col items-center justify-center gap-4">
              <div className="p-4 bg-orange-55 rounded-full border border-orange-200 text-orange-655 animate-pulse">
                <Compass className="w-10 h-10 animate-spin" style={{ animationDuration: '4s' }} />
              </div>
              <div>
                <h3 className="text-lg font-bold text-slate-800 font-quicksand">Kunjungan Klien Sedang Aktif</h3>
                <p className="text-sm text-slate-500 mt-2 max-w-md mx-auto leading-relaxed font-medium">
                  Anda sedang terdaftar dalam Kunjungan Klien hari ini.
                  <br />
                  Jika seluruh agenda pertemuan klien/mitra kerja Anda hari ini telah selesai, silakan tekan tombol **Akhiri Kunjungan** di bawah untuk membuka formulir **Absen Keluar (Check-Out)**.
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <section className="bg-white border border-orange-100 rounded-3xl p-6 shadow-sm space-y-6">
              <div className="border-b border-orange-100 pb-3 flex justify-between items-center">
                <div>
                  <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2 font-quicksand">
                    <Camera className="w-5 h-5 text-red-500" />
                    Formulir Presensi: Keluar Klien (Check-Out)
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">Harap ambil foto wajah dan aktifkan lokasi GPS Anda.</p>
                </div>
                <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {latitude && longitude && (
                      <div className="p-3 rounded-xl border text-[11px] font-bold flex items-start gap-2 leading-relaxed text-emerald-700 bg-emerald-50 border-emerald-200">
                        <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                        <span>Lokasi siap untuk absensi Keluar Klien. Radius kantor dinonaktifkan.</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="space-y-2 pt-2 border-t border-orange-100">
                ...
                <textarea placeholder="Tambahkan pesan laporan singkat akhir kunjungan klien..." value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} className="w-full bg-slate-50 border border-slate-200 focus:border-red-500 text-slate-800 placeholder-slate-400 rounded-xl py-2.5 px-4 outline-none transition-all text-xs resize-none font-medium font-quicksand" />
              </div>
              ...
              <div className="pt-2 flex justify-end">
                <button onClick={() => handleAttendanceSubmit('check-out')} disabled={submitting || !capturedPhoto || !latitude || !longitude} className="px-6 py-3 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-2xl transition-all shadow-lg shadow-orange-600/10 cursor-pointer text-sm flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed font-quicksand">
                  {submitting ? (
                    ...
                    <>
                      <Check className="w-4 h-4" /> Kirim Absen Keluar Klien
                    </>
                  )}
                </button>
              </div>
          ...
          <section className="bg-white border border-orange-100 rounded-3xl p-6 shadow-sm space-y-6 relative overflow-hidden">
            ...
            <div className="flex justify-between items-center pb-3 border-b border-orange-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800 font-quicksand">Absen Keluar Klien Tercatat!</h2>
                  <p className="text-xs text-slate-500">Anda sudah melakukan absen keluar kunjungan klien untuk hari ini.</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 pt-2">
              <div className="md:col-span-7 space-y-4">
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
                  <div>
                    <span className="text-[10px] font-bold text-slate-550 uppercase tracking-wider block font-quicksand">Waktu Keluar Klien</span>
