    ...
    <div className="flex flex-col h-full justify-between">
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 px-3 py-2 border-b border-slate-800/60 pb-5">
          ...
          <div>
            <h2 className="text-sm font-bold text-white leading-none font-quicksand">Portal Karyawan</h2>
        ...
        <div className="bg-slate-950/40 border border-slate-800/80 rounded-2xl p-4 flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-slate-850 flex items-center justify-center text-slate-350 font-bold text-sm">
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <ChevronRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-all text-slate-650" />
              </NavLink>
            )
          })}
        </nav>
      ...
      <div className="pt-6 border-t border-slate-800/60 space-y-3">
        <div className="flex items-center gap-2 text-[10px] text-slate-500 font-bold font-mono px-3">
          ...
          <span>Karyawan Access</span>
        </div>
        <button
          onClick={onLogout}
          className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-slate-950 border border-slate-850 hover:bg-rose-500/10 hover:border-rose-500/20 text-slate-400 hover:text-rose-455 rounded-xl text-xs font-bold transition-all cursor-pointer font-quicksand"
        >
